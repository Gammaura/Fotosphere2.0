...
async function finalizeStrip(){
    ...
    try{const r=await fetch(`/api/session/${S.sid}/finalize`,{method:'POST',body:fd});if(!r.ok)throw new Error('Finalize failed');const d=await r.json();S.stripUrl=d.strip_url||'';S.gifUrl=d.gif_url||'';$('done-img').src=S.stripUrl;if(d.qr_b64)$('done-qr').innerHTML=`<img src="data:image/png;base64,${d.qr_b64}">`;noloader();stopTimer();show('done');doneToggle('photo')}catch(e){noloader();showModal('Gagal',e.message,'⚠️')}
}
